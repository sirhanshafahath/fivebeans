...
FiveBeansWorker.prototype.doWork = function doWork(handler, jobID, jobdata)
{
    ...
    self.client.stats_job(jobID, function (e, stats) {
      if (e) {
		self.emitWarning({ message: 'job stats error', id: jobID, error: e });
		self.buryAndMoveOn(jobID);
      }
      else {
        var touchInterval = stats.ttr*1000 - 1500;
        if (touchInterval < 20000) touchInterval = 20000;
        self.emitInfo(JSON.stringify(stats));
        self.interval = setInterval(function () {
                                    self.client.touch(jobID, function (e) {
                                      if (e) {
                                        self.emitWarning({ message: 'job touch error', id: jobID, error: e });
                                        self.buryAndMoveOn(jobID);
                                      }
                                      else {
                                        self.emitInfo('touched jobID: ' . jobID);
                                      }
                                    });
                                  }, touchInterval);
        try
